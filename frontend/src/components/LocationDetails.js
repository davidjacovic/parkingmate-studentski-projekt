...
function LocationDetails() {
  ...
  // Fetch tariffs for this location
  useEffect(() => {
    fetch(`http://localhost:3002/tariffs/by-location/${id}`)
      .then(res => {
        if (!res.ok) throw new Error('Neuspešno učitavanje tarifa');
        return res.json();
      })
      .then(data => setTariffs(data))
      .catch(err => {
        console.error(err);
        setTariffsError('Greška pri učitavanju tarifa.');
      });
  }, [id]);
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '2rem auto', textAlign: 'left' }}>
      ...
      <p><strong>Opis:</strong> {location.description || 'Nema opisa za ovu lokaciju.'}</p>
      <p>
      ...
      <div style={{ marginTop: '2rem' }}>
        <h3>Tarife</h3>
        {tariffsError && <p style={{ color: 'red' }}>{tariffsError}</p>}
        {!tariffsError && tariffs.length === 0 && <p>Nema dostupnih tarifa za ovu lokaciju.</p>}
        {!tariffsError && tariffs.length > 0 && (
          <ul>
            {tariffs.map(tariff => (
              <li key={tariff._id} style={{ marginBottom: '1rem' }}>
                <strong>Tip tarife:</strong> {tariff.vehicle_type || 'Nije definisano'}<br />
                <strong>Trajanje:</strong> {tariff.duration || 'Nije definisano'}<br />
                <strong>EM:</strong> {tariff.price_unit || 'Nije definisano'}<br />
                <strong>Tip vozila:</strong> {tariff.tariff_type || 'Nije definisano'}<br />
                <strong>Cena:</strong> {tariff.price ? parseFloat(tariff.price.$numberDecimal).toFixed(2) : 'Nije definisano'} {tariff.price_unit || ''}<br />
              </li>
            ))}
          </ul>
        )}
      ...
      {userLocation && (
        <div style={{ height: '400px', width: '100%', marginTop: '2rem' }}>
          ...
          >
            ...
            <Routing
              from={userLocation}
              to={[location.location.coordinates[1], location.location.coordinates[0]]}
            />
          </MapContainer>
        </div>
      )}
