 ...
 * Supports Geoapify and Mapbox tile services.
 */
public class MapRasterTiles {
     ...
     * Get raster tile based on zoom and tile number.
     *
     * @param zoomXY ZoomXY object containing zoom, x, y
     * @return Texture of the tile
     * @throws IOException if tile cannot be fetched
     */
     ...
     * @param size   number of tiles in one dimension (size x size grid)
     ...
     * @return ByteArrayOutputStream containing tile data
     * @throws IOException if tile cannot be fetched
     */
     ...
     *
     * @param array byte array containing image data
     ...
     * @param lon  longitude
     * @param zoom zoom level
     * @return ZoomXY object with tile coordinates
     */
     ...
     * @return Vector2 with pixel coordinates
     */
    ...
    /**
     * Converts geolocation to pixel position using default constants.
     *
     * @param lat       latitude
     * @param lng       longitude
     * @param beginTileX x (tile number) of top left tile
    ...
    public static Vector2 getPixelPosition(double lat, double lng, int beginTileX, int beginTileY) {
        ...
        // Scale to fit our image
        double scale = Math.pow(2, MapConstants.ZOOM);

        // Apply scale to world coordinates to get image coordinates
        return new Vector2(
                (int) (Math.floor(worldCoordinate[0] * scale) - (beginTileX * MapRasterTiles.TILE_SIZE)),
                MapConstants.MAP_HEIGHT - (int) (Math.floor(worldCoordinate[1] * scale) - (beginTileY * MapRasterTiles.TILE_SIZE) - 1)
        );
    }

    /**
     * Fetches a route from coordinates using Geoapify routing API.
     *
     * @param geolocations array of geolocations to route through
     * @return array of geolocation arrays representing the route segments
     */
    ...
    /**
     * Gets route from coordinates using Geoapify routing API.
     *
     * @param coordinates array of [lat, lng] coordinate pairs
     * @return array of geolocation arrays representing the route segments
     * @throws Exception if route cannot be fetched
     */
